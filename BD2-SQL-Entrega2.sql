...
SELECT 
    Pilotos.*, 
    (SELECT COUNT(DISTINCT Corridas.cod_Pista) 
     FROM Corridas, Vencedores 
     WHERE Vencedores.cod_Piloto = Pilotos.cod_Piloto 
     AND Vencedores.cod_Corrida = Corridas.cod_Corrida) AS pistas 
FROM Pilotos 
ORDER BY pistas DESC
...
SELECT 
    Pilotos.*, 
    (SELECT COUNT(DISTINCT Corridas.cod_Temporada) 
     FROM Corridas, Vencedores 
     WHERE Vencedores.cod_Piloto = Pilotos.cod_Piloto 
     AND Vencedores.cod_Corrida = Corridas.cod_Corrida) AS temporadas 
FROM Pilotos 
ORDER BY temporadas DESC
INTO OUTFILE '/var/lib/mysql/csv/Pergunta2.csv'
FIELDS TERMINATED BY ';' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
...
SELECT 
    Construtores.*, 
    (SELECT COUNT(DISTINCT Contratos.cod_Piloto) 
     FROM Contratos
     WHERE Construtores.cod_Construtor = Contratos.cod_Construtor) AS pilotos 
FROM Construtores 
ORDER BY pilotos DESC
...
SELECT 
    Temporadas.*, 
    (SELECT COUNT(DISTINCT Vencedores.cod_Piloto) 
     FROM Corridas, Vencedores 
     WHERE Temporadas.cod_Temporada = Corridas.cod_Temporada 
     AND Corridas.cod_Corrida = Vencedores.cod_Corrida) AS vencedores 
FROM Temporadas 
ORDER BY vencedores DESC
...
SELECT 
    Temporadas.*, 
    (SELECT COUNT(Corridas.cod_Temporada) 
     FROM Corridas
     WHERE Temporadas.cod_Temporada = Corridas.cod_Temporada) AS corridas 
FROM Temporadas 
ORDER BY corridas DESC
LIMIT 1
...
SELECT 
    ...
    (SELECT COUNT(DISTINCT Vencedores.cod_Piloto) 
     FROM Corridas, Vencedores 
     WHERE Pistas.cod_Pista = Corridas.cod_Pista 
     AND Corridas.cod_Corrida = Vencedores.cod_Corrida) AS vencedores 
FROM Pistas 
ORDER BY vencedores DESC
...
SELECT 
    Construtores.*, 
    (SELECT SUM(Classificacoes.segundo_Lugar) 
     FROM Contratos, Classificacoes 
     WHERE Construtores.cod_Construtor = Contratos.cod_Construtor 
     AND Contratos.cod_Piloto = Classificacoes.cod_Piloto) AS segundos_lugares 
FROM Construtores 
ORDER BY segundos_lugares DESC
INTO OUTFILE '/var/lib/mysql/csv/Pergunta7.csv'
FIELDS TERMINATED BY ';' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
...
SELECT 
    ...
    COUNT(*) AS num_pilotos 
FROM Pilotos 
GROUP BY Pilotos.nacionalidade
INTO OUTFILE '/var/lib/mysql/csv/Pergunta8.csv'
FIELDS TERMINATED BY ';' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
...
GROUP BY Pilotos.nacionalidade
ORDER BY total_vitorias DESC, total_poles DESC, total_voltas_rapidas DESC, total_titulos DESC
INTO OUTFILE '/var/lib/mysql/csv/Pergunta9.csv'
FIELDS TERMINATED BY ';' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
